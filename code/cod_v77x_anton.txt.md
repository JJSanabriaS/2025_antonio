...
def get_vencimentos(driver, url="https://opcoes.net.br/opcoes/bovespa/AMBP3"):
    ...
    try:
        ...
        print(f"✅ {len(vencimentos)} vencimentos detectados.")
        return vencimentos
    except Exception as e:
        print("❌ Erro ao detectar vencimentos:", e)
        return []

def set_vencimentos(driver, ativo, selected_ids):
    """Abre ativo, limpa seleções anteriores e aplica novos vencimentos."""
    ...
    try:
        ...
        for cb in checkboxes:
            if cb.get_attribute("id") in selected_ids:
                driver.execute_script("arguments[0].click();", cb)
        print("⏳ Aguardando atualização da página (3s)...")
        time.sleep(3)
    except Exception as e:
        print("❌ Erro ao configurar vencimentos:", e)
...
def armazem(df, name):
    ...
    print(f"💾 Arquivos '{name}.csv' e '{name}.xlsx' gerados.")
    return name
...
def prinp(url2):
    """Extrai dados de um ativo específico."""
    print(f"\n==== Iniciando {url2} ====")
...
def get_sel(driver, string):
    """Percorre todas as opções ou um ativo específico."""
...
try:
    ...
    select_element = WebDriverWait(driver, 15).until(
        EC.presence_of_element_located((By.NAME, "IdLista"))
    )

    # Seleciona a opção de valor "TA"
    select = Select(select_element)
    select.select_by_value("TA")

    #print("✅ Campo 'IdLista' definido para 'TA' com sucesso.")
except Exception as e:
    print(f"❌ Erro ao definir IdLista: {e}")
...
def on_obter_clicked(b):
    ...
    with output:
        ...
        if not selected:
            print("⚠️ Selecione pelo menos um vencimento.")
            return
...
def on_baixar_clicked(b):
    with output:
        ...
        try:
            ...
            print("❌ Arquivos ainda não gerados:", e)
