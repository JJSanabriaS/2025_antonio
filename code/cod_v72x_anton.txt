...
if not final_df.empty and "Formula" in final_df.columns:
    # Filtro adicional baseado no valor mínimo entre Bid e Ask
    try:
        #user_threshold = float(input("Digite o valor mínimo permitido entre Bid e Ask (ex: 0.05): "))
        final_df["MinBidAsk"] = final_df[["Bid", "Ask"]].min(axis=1)
        final_df = final_df[final_df["MinBidAsk"] >= user_threshold]
        print(f"\n🔎 Filtradas {len(final_df)} linhas com Min(Bid, Ask) >= {user_threshold}")
    except Exception:
        print("⚠️ Nenhum filtro aplicado (erro ou valor inválido).")
