...
def run_scraper(b):
    ...
    with out:
        ...
        # Clear all checkboxes in #listadevenximentos
        try:
            container = driver.find_element(By.ID,"listadevencimentos")
            for cb in container.find_elements(By.TAG_NAME,"input"):
                if cb.is_selected(): cb.click()
        except:
            print("⚠️ Não foi possível limpar checkboxes.")
        # Check only selected vencimentos
        for v_id in venc_sel:
            try:
                cb = driver.find_element(By.ID,v_id)
                if not cb.is_selected(): cb.click()
            except:
                print(f"⚠️ Erro ao selecionar {v_id}")
        time.sleep(2)
